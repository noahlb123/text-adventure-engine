...
2. At the beggining of the In the file define a scene name preceeded by `#`:
...
5. After step 4, a scene is complete. The final step is to define new scenes for each option, ensuring that the new scene names exactly match the ones defined in the options:
...
6. At this point you can upload the .txt file to the site and expirience the user prespective.
...
/Unfortunitley, the clock is ticking

the hours are going by. The past increases, the future receeds. Possibilities decreasing, regrets mounting:
...
Body text is the only thing the user will see in an scene, unless there are options.

 

Requirements:

* Must be preceeded by a scene name
...
* Must be preceeded by body text
...
Whitespace does matter in the text engine, but it it can also be flexible depending on the circumstance. Specificly, the JavaScript `trim()` function is used on each component of a scene, which makes leading and trailing whitespace irrelevent, but means that internal whitespace does matter.
...
The characters used to define each scene component (`#`, `/`, `>`, `:`) cannot be used as normal. Instead these characters must be preceeded by the escape character `\` in order for them to function properly within the text of a scene component. Additionlly, to display `\` in text, it must be preceeded with an additional `\`:
...
http\:\/\/en.wikipedia.org\/wiki\/List_of_Unicode_characters\#Phonetic_scripts
